...
| 文件 | 导出 | 说明 |
|------|------|------|
| `admin.go` | `NewServer(Options)`、`NewHandler(Options)`、`Options` | 独立端口（默认 `:9090`）暴露 `/metrics`（Prometheus）、`/healthz`（存活）、`/readyz`（就绪，支持 `ReadinessCheck` 回调）、`/debug/pprof`（按配置开关） |
...
| 换真实存储 | 给 `UserServer` 加 DB/Redis 依赖字段 + 构造函数注入，替换内存 map |
